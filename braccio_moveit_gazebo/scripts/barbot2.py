...
def kinematic(x,y,z):
    # Ref https://github.com/Phylliade/ikpy
    # Set your target location (x,y,z), in meters
    # target = [ 0.2, 0.2, 0.2 ]
    target = [x,y,z]
    # Load the chain from the model
    # We mask out the first link, ‘cos it doesn’t move.
    chain = ikpy.chain.Chain.from_urdf_file("/home/marc/Documents/braccio_arm_ws/src/braccio_moveit_gazebo/braccio_description/urdf/braccio_model.urdf", None, None, 'link', [ False, True, True, True, True, True, True ])

    # Calculate the joint angles for the target location.
    # [0] * 3 is the orientation.  Idk what the reference frame is.
    ik = chain.inverse_kinematics(target, [0] * 3, optimizer="scalar")

    # Print it out.
    # First the calculated joint angles
    print("Joint angles for target {target}")
    for a, b in zip(chain.links,ik):
      print(f"{a.name}: {b}")
    print("")
    
    # Convert Zip to list of b_values
    j_values = [b for _, b in zip(chain.links, ik)]
    
    #print j_values for sanity
    print("jvalues list")
    print(j_values)
    print("")


    # And then reverse the calculation.  If all went well, we should be back at the target
    print("Forward kinematics for these joint angles:")
    forward = chain.forward_kinematics(ik)
    print(forward)
    print(f"End position: {forward[:3,3]}")
 
    # Return the joint required angles - ignoring the first value for base link
    return(j_values)
...
class BraccioObjectTargetInterface(object):
  ...
  def go_to_0(self):
    self.go_to_j(j0=0, j1=0,j2=0,j3=0)

  def go_to_45(self):
    self.go_to_j(j0=0.76, j1=0.76,j2=0.76,j3=0.76)

  def go_to_90(self):
    self.go_to_j(j0=1.57, j1=1.57,j2=1.57,j3=1.57)

  def go_to_135(self):
    self.go_to_j(j0=2.36, j1=2.36,j2=2.36,j3=2.36)

  def go_to_180(self):
    self.go_to_j(j0=3.14, j1=3.14,j2=3.14,j3=3.14)

  def listen_to_rick(self):
    print ('pos x?')
    tst = input()
    if tst!='':
        x = float(tst)
    ...
    joint_values = kinematic(x,y,z)
    
    #joint_values = kinematic(0.2,0.2,0.2)
    self.go_to_j(j0=joint_values[1], j1=joint_values[2],j2=joint_values[3])
  ...
  def go_to_xy(self, x, y, r, how):
    if how=='top':
      ...
      if joint_targets[0]<0 or joint_targets[0]>3.14:
        print ('++++++ Not in reachable area, aborting - ERR 001++++++')
        return -1
      if np.isnan(joint_targets[1]) and s < S_TOP_MAX and s > S_SIDE_MIN:
        print ('++++++ Too far out, pulling backwards ERR 002 +++++')
        self.go_to_pull(joint_targets[0])
        return 1
      if np.isnan(joint_targets[1]):
        print ('++++++ Not in reachable area, aborting ERR 003++++++')
        return -1
    elif how=='side':
      s, joint_targets = self.get_targets(x,y)
      print (joint_targets)
      if joint_targets[0]<0 or joint_targets[0]>3.14:
        print ('++++++ Not in reachable area, aborting ERR 004 ++++++')
        return -1
      if np.isnan(joint_targets[1]) and s < S_SIDE_MAX and s > S_SIDE_MIN:
        print ('++++++ Too close, pushing backwards ERR 005+++++')
        self.go_to_push(joint_targets[0])
        return 1
      if np.isnan(joint_targets[1]):
        print ('++++++ Not in reachable area, aborting ERR 006++++++')
        return -1
...
def print_instructions():
  ...
  print ("1 = Move to 0 deg test position")
  print ("2 = Move to 45 deg test position")
  print ("3 = Move to 90 deg test position")
  print ("4 = Move to 135 deg test position")
  print ("5 = Move to 180 deg test position")
  print ("R = Ricks IK model")
...
""")
  ...
  while True:
      ...
      if inp=='1':
          bb_targetter.go_to_0()
      if inp=='2':
          bb_targetter.go_to_45()
      if inp=='3':
          bb_targetter.go_to_90()
      if inp=='4':
          bb_targetter.go_to_135()
      if inp=='5':
          bb_targetter.go_to_180()
      if inp=="g":
          bb_targetter.gripper_close()
      if inp=="h":
          bb_targetter.gripper_open()          
      if inp=='R':
          bb_targetter.listen_to_rick()
      if inp=='J':
      	  bb_targetter.joystick_control()
